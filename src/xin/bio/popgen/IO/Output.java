...
public class Output {
	...
	public static void log(int variantSize, int indSize, int popSize, String variantType) {
	 ...
	 * @param snpId          the name of the SNP
	 * @param popiId         the population name of the i-th population
	 * @param popjId         the population name of the j-th population
	 ...
	 * @param countBw        countBw counts of reference allele in population B
	 * @param countBm        countBm counts of reference allele in population B
	 * @param varOmega       the variance of Omega between the i-th population and the j-th population
	 * @param divergenceTime the divergence time between the i-th population and the j-th population
	 * @param chisq          a ChiSquaredDistribution class for calculating p-value
	 * @param bw             a BufferedWriter class for output results
	 */
	public static void getResults(GeneticVariant g, String popiId, String popjId, 
		...
		for (int i = 0; i < popaAlleleCount.length; i++) {
			...
			if ((countAw + countAm != 0) && (countBw + countBm != 0)) {
				...
				varLogOdds      = Model.calVarLogOdds(countAw, countAm, countBw, countBm);
			...
			output.add(g.getId())
				  ...
				  .add(g.getAllele(i))
			      .add(popiId)
			      .add(String.valueOf((int) countAw))
			      .add(String.valueOf((int) countAm))
			      .add(popjId)
